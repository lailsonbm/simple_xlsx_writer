...
ends
    @doc.sheets.each_with_index do |sheet, ndx|
      @zip.puts "<sheet name=\"#{sheet.name}\" sheetId=\"#{ndx + 1}\" r:id=\"#{sheet.rid}\"/>"
    end
...
ends
    ...
    @doc.sheets.each_with_index do |sheet, ndx|
      @zip.puts "<Override PartName=\"/xl/worksheets/sheet#{ndx+1}.xml\" ContentType=\"application/vnd.openxmlformats-officedocument.spreadsheetml.worksheet+xml\"/>"
    end
...
ends
    cnt = 0
    @zip.puts "<Relationship Id=\"rId#{cnt += 1}\" Type=\"http://schemas.openxmlformats.org/officeDocument/2006/relationships/styles\" Target=\"styles.xml\"/>"
    @doc.sheets.each_with_index do |sheet, ndx|
      sheet.rid = "rId#{cnt += 1}"
      @zip.puts "<Relationship Id=\"#{sheet.rid}\" Type=\"http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet\" Target=\"worksheets/sheet#{ndx + 1}.xml\"/>"
    end
    if @doc.has_shared_strings?
